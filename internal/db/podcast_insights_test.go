...
func TestDeletePodcastInsightsByURL(t *testing.T) {
	d := newTestDB(t)

	// Deleting a URL with no rows is a harmless no-op.
	if err := d.DeletePodcastInsightsByURL("https://vocus.cc/article/example"); err != nil {
		t.Fatalf("DeletePodcastInsightsByURL() on empty table error = %v", err)
	}
	...
	if err := d.DeletePodcastInsightsByURL("https://vocus.cc/article/example"); err != nil {
		t.Fatalf("DeletePodcastInsightsByURL() error = %v", err)
	}

	if count, err := d.CountPodcastInsightsByURL("https://vocus.cc/article/example"); err != nil {
		t.Fatalf("CountPodcastInsightsByURL() after delete error = %v", err)
	} else if count != 0 {
		t.Fatalf("CountPodcastInsightsByURL() after delete = %d, want 0", count)
	}
	// The other episode's rows must survive — this deletes by exact URL,
	// not the whole table.
	if count, err := d.CountPodcastInsightsByURL("https://vocus.cc/article/other"); err != nil {
		t.Fatalf("CountPodcastInsightsByURL() (other) after delete error = %v", err)
	} else if count != 1 {
		t.Fatalf("CountPodcastInsightsByURL() (other) after delete = %d, want 1 (untouched)", count)
	}
}
