...
var enMessages = map[Key]string{
	...
	KeyTWUniverseRefreshed:           "🔄 *TW Scan Pool Rotated*\n\nTop 300 by turnover over the last 5 sessions: %d added, %d dropped.\n\nAdded: %s",
	...
	KeyUniverseUsage:           "Usage: /universe [add|remove] TICKER | /universe refresh\nNo arguments shows a summary of the scan pool.",
	KeyUniverseRefreshStart:    "🔄 Fetching turnover rankings for the last 5 sessions from Shioaji…",
	KeyUniverseRefreshFailed:   "❌ Scan pool rotation failed: %v",
	KeyUniverseRefreshNoDaemon: "⚠️ SHIOAJI_ADDR is not set, so there is no turnover ranking source; the scan pool is unchanged.",
	...
	KeyStrategyUnvalidatedMTF:             "ℹ️ TW measurement (2026-09-04, within-ticker): P(20-day return > 10%) is +4.8pp (2.7 sigma) early and +5.7pp (3.2 sigma) late; hit rate 22.0%, base rate 14.6%. But the 20/30 degree thresholds were picked with both splits visible, so the split test is no longer out-of-sample for them; and 60 of 116 tickers are positive against 56 negative, while dropping the best 5%: the mean 20-day return goes +2.90% -> +0.43%. The edge is a right tail, so cherry-picking gives it back. Watchlist material only, not an entry trigger on its own.",
	KeyStrategyMTFCrossUS:                 "⚠️ US is observation only: this screen measures NEGATIVE on US (P(20-day return > 10%) is -6.9pp / 10.9 sigma early and -3.7pp / 4.8 sigma late, 2026-09-04, within-ticker). At the trigger the median name is already up 20%, measured over the last 60 sessions, and 6 in 10 have just made a 1-year high: late in a move rather than the start. It keeps firing because you asked to watch it, but on US it is a contrarian indicator.",
...
`,
	...
	KeyEvalSourceTitle:           "\n[By source: source x action x horizon]\n",
	KeyEvalMarketTitle:           "\n[Market: %s]\n",
	...
	KeyEvalCountsTitle:           "\n[Honest counts]\n",
	KeyEvalCountHold:             "  Not scored (no action): %d\n",
	KeyEvalCountCollapsed:        "  Consecutive repeats merged (first kept): %d\n",
	...
	KeyStrategyMTFCross:      "📈 %s triggered [Daily-Weekly MA Cross] (%s): daily MA5/MA7 angle >=20 deg, weekly MA5/MA7 angle >=30 deg, RSI <=80",
